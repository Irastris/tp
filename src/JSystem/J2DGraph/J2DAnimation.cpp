...
void J2DAnmTransformKey::calcTransform(f32 param_0, u16 param_1, J3DTransformInfo* transformInfo) const {
    u16 idx = param_1 * 3;
...
void J2DAnmColorKey::getColor(u16 param_0, GXColor* pColor) const {
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mRInfo, &mRValues[info->mRInfo.mOffset]);
        if (val < 0) {
            pColor->r = 0;
        } else if (val > 255) {
            pColor->r = 255;
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mGInfo, &mGValues[info->mGInfo.mOffset]);
        if (val < 0) {
            pColor->g = 0;
        } else if (val > 255) {
            pColor->g = 255;
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mBInfo, &mBValues[info->mBInfo.mOffset]);
        if (val < 0) {
            pColor->b = 0;
        } else if (val > 255) {
            pColor->b = 255;
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mAInfo, &mAValues[info->mAInfo.mOffset]);
        if (val < 0) {
            pColor->a = 0;
        } else if (val > 255) {
            pColor->a = 255;
...
void J2DAnmTextureSRTKey::calcTransform(f32 param_0, u16 param_1, J3DTextureSRTInfo* pInfo) const {
    ...
    case 1:
        ...
        break;
    default:
        pInfo->mScaleX = J2DGetKeyFrameInterpolation<f32>(param_0, &xInf->mScaleInfo, &mScaleValues[xInf->mScaleInfo.mOffset]);
    }
    switch (yInf->mScaleInfo.mMaxFrame) {
    case 0:
        pInfo->mScaleY = 1;
    ...
    case 1:
        ...
        break;
    default:
        pInfo->mScaleY = J2DGetKeyFrameInterpolation<f32>(param_0, &yInf->mScaleInfo, &mScaleValues[yInf->mScaleInfo.mOffset]);
    }
    switch (zInf->mRotationInfo.mMaxFrame) {
    case 0:
        pInfo->mRotation = 0;
...
void J2DAnmTevRegKey::getTevColorReg(u16 param_0, GXColorS10* pColor) const {
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mRTable, &mCRValues[info->mRTable.mOffset]);
        if (val < -0x400) {
            pColor->r = -0x400;
        } else if (val > 0x3FF) {
            pColor->r = 0x3FF;
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mGTable, &mCGValues[info->mGTable.mOffset]);
        if (val < -0x400) {
            pColor->g = -0x400;
        } else if (val > 0x3FF) {
            pColor->g = 0x3FF;
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mBTable, &mCBValues[info->mBTable.mOffset]);
        if (val < -0x400) {
            pColor->b = -0x400;
        } else if (val > 0x3FF) {
            pColor->b = 0x3FF;
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mATable, &mCAValues[info->mATable.mOffset]);
        if (val < -0x400) {
            pColor->a = -0x400;
        } else if (val > 0x3FF) {
            pColor->a = 0x3FF;
...
void J2DAnmTevRegKey::getTevKonstReg(u16 param_0, GXColor* pColor) const {
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mRTable, &mKRValues[info->mRTable.mOffset]);
        if (val < 0) {
            pColor->r = 0;
        } else if (val > 255) {
            pColor->r = 255;
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mGTable, &mKGValues[info->mGTable.mOffset]);
        if (val < 0) {
            pColor->g = 0;
        } else if (val > 255) {
            pColor->g = 255;
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mBTable, &mKBValues[info->mBTable.mOffset]);
        if (val < 0) {
            pColor->b = 0;
        } else if (val > 255) {
            pColor->b = 255;
    ...
    case 1:
        ...
        break;
    default:
        val = J2DGetKeyFrameInterpolation<s16>(mFrame, &info->mATable, &mKAValues[info->mATable.mOffset]);
        if (val < 0) {
            pColor->a = 0;
        } else if (val > 255) {
            pColor->a = 255;
