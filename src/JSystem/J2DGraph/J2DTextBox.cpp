...
J2DTextBox::J2DTextBox() : mFont(NULL), mCharColor(), mGradientColor(), mStringPtr(NULL), mWhiteColor(), mBlackColor() {
    initiate(NULL, NULL, 0, HBIND_LEFT, VBIND_TOP);
}

J2DTextBox::J2DTextBox(J2DPane* p_pane, JSURandomInputStream* p_stream, JKRArchive* p_archive) : mFont(NULL), mCharColor(), mGradientColor(), mStringPtr(NULL), mWhiteColor(), mBlackColor() {
    private_readStream(p_pane, p_stream, p_archive);
}

J2DTextBox::J2DTextBox(J2DPane* p_pane, JSURandomInputStream* p_stream, u32 param_2, J2DMaterial* p_material) : mFont(NULL), mCharColor(), mGradientColor(), mStringPtr(NULL), mWhiteColor(), mBlackColor() {
    J2DTextBoxInfo info;
...
J2DTextBox::J2DTextBox(u64 tag, JGeometry::TBox2<f32> const& bounds, ResFONT const* p_font, char const* string, s16 strLength, J2DTextBoxHBinding hBind, J2DTextBoxVBinding vBind) : J2DPane(tag, bounds), mFont(NULL), mCharColor(), mGradientColor(), mStringPtr(NULL), mWhiteColor(), mBlackColor() {
    initiate(p_font, string, strLength, hBind, vBind);
}

void J2DTextBox::initiate(ResFONT const* p_font, char const* string, s16 length, J2DTextBoxHBinding hBind, J2DTextBoxVBinding vBind) {
    if (p_font != NULL) {
...
void J2DTextBox::private_readStream(J2DPane* p_pane, JSURandomInputStream* p_stream, JKRArchive* p_archive) {
...
void J2DTextBox::draw(f32 posX, f32 posY, f32 param_2, J2DTextBoxHBinding hBind) {
    ...
    if (isVisible()) {
        J2DPrint print(mFont, mCharSpacing, mLineSpacing, mCharColor, mGradientColor, mBlackColor, mWhiteColor);
        print.setFontSize(mFontSizeX, mFontSizeY);
        makeMatrix(posX, posY, 0.0f, 0.0f);
        ...
        if (mStringPtr != NULL) {
            print.printReturn(mStringPtr, param_2, 0.0f, hBind, VBIND_TOP, 0.0f, -mFontSizeY, mAlpha);
        }
        MTXIdentity(m);
        GXLoadPosMtxImm(m, 0);
    }
...
void J2DTextBox::drawSelf(f32 param_0, f32 param_1, Mtx* p_mtx) {
    ...
    if (mStringPtr != NULL) {
        print.printReturn(mStringPtr, mBounds.getWidth() + 0.0001f, mBounds.getHeight(), getHBinding(), getVBinding(), field_0x10c, field_0x110, mColorAlpha);
    }
}
