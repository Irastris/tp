...
void fpcM_Management(fpcM_ManagementFunc i_preExecuteFn, fpcM_ManagementFunc i_postExecuteFn) {
    ...
    if (!dShutdownErrorMsg_c::execute()) {
        ...
        if (!dDvdErrorMsg_c::execute()) {
            ...
            // TODO: Next two lines used to be behind some checks, determine if they should be retained
...
base_process_class* fpcM_FastCreate(s16 i_procname, FastCreateReqFunc i_createReqFunc, void* i_createData, void* i_append) {
    return fpcFCtRq_Request(fpcLy_CurrentLayer(), i_procname, (fstCreateFunc)i_createReqFunc, i_createData, i_append);
}
