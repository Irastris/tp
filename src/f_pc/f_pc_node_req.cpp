...
int fpcNdRq_phase_Create(node_create_request* i_request) {
    i_request->creating_id = fpcSCtRq_Request(i_request->layer, i_request->name, (stdCreateFunc)i_request->create_req_methods->post_method, i_request, i_request->data);
...
int fpcNdRq_Cancel(node_create_request* i_request) {
    if (i_request->create_req_methods != NULL && fpcMtd_Method(i_request->create_req_methods->cancel_method, i_request) == 0) {
        return 0;
    }
...
int fpcNdRq_IsPossibleTarget(process_node_class* i_procNode) {
    ...
    while (req_node != NULL) {
        ...
        if ((create_req->parameters == 2 || create_req->parameters == 4 || create_req->parameters == 1) && create_req->node_proc.id == id) {
            return 0;
        }
...
node_create_request* fpcNdRq_Request(u32 i_requestSize, int i_reqType, process_node_class* i_procNode, s16 i_procName, void* i_data, node_create_request_method_class* i_create_req_methods) {
