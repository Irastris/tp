...
int fpcPi_Handler() {
    ...
    while (i_procPriority = fpcPi_GetFromQueue()) {
        ...
        if (fpcLyTg_Move(pLayerTag, i_procPriority->queue_info.layer_id, i_procPriority->queue_info.list_id, i_procPriority->queue_info.list_priority) == 1) {
...
int fpcPi_Init(process_priority_class* i_procPriority, void* i_data, fpc_ProcID i_layerID, u16 i_listID, u16 i_priority) {
    if (!fpcPi_IsNormal(i_layerID, i_listID, i_priority)) {
        return 0;
    }
