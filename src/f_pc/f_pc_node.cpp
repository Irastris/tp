...
int fpcNd_Delete(process_node_class* i_procNode) {
    if (!fpcLy_IsDeletingMesg(&i_procNode->layer) && fpcMtd_Delete(&i_procNode->nodedraw_method->base, i_procNode) == 1) {
...
int fpcNd_Create(process_node_class* i_procNode) {
    ...
    if (i_procNode->base.state.init_state == 0) {
        node_process_profile_definition* pprofile = (node_process_profile_definition*)i_procNode->base.profile;
        i_procNode->base.subtype = fpcBs_MakeOfType(&g_fpcNd_type);
        i_procNode->nodedraw_method = (nodedraw_method_class*)pprofile->sub_methods;
