...
inline bool cLib_IsZero(f32 value) { return fabsf(value) < 8e-11f; }
...
template <typename T>
inline void cLib_offBit(T& value, T bit) { value = (T)(value & ~bit); }

template <typename T>
inline void cLib_onBit(T& value, T bit) { value = (T)(value | bit); }

template <typename T>
inline T cLib_checkBit(T value, T bit) { return (T)(value & bit); }

template <typename T>
inline T cLib_minMaxLimit(T val, T min, T max) { return (T)(val < min ? min : (val > max ? max : val)); }
...
template <typename T>
T cLib_getRndValue(T min, T range) { return (T)(min + cM_rndF((f32)range)); }
